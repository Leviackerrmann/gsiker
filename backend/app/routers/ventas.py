...
from app.models.inventario import MotivoMovimiento, MovimientoInventario, TipoMovimiento, Bodega
from app.services.inventario import actualizar_stock
from app.services.ventas import generar_numero_ov, generar_numero_factura, procesar_despacho, generar_factura
...
@router.get("/clientes")
async def list_clientes(db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
...
async def create_cliente(body: dict, db: AsyncSession = Depends(get_db), _=Depends(require_admin)):
    if (await db.execute(select(Cliente).where(Cliente.codigo == body["codigo"]))).scalar_one_or_none():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="El código ya existe")
    c = Cliente(codigo=body["codigo"], nombre=body["nombre"], documento=body.get("documento"),
...
async def update_cliente(cliente_id: int, body: dict, db: AsyncSession = Depends(get_db), _=Depends(require_admin)):
    c = (await db.execute(select(Cliente).where(Cliente.id == cliente_id))).scalar_one_or_none()
    if not c: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Cliente no encontrado")
    for k, v in body.items(): setattr(c, k, v)
...
async def create_cotizacion(body: dict, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    result = await db.execute(select(func.count()).select_from(CotizacionVenta))
    numero = f"COTV-{(result.scalar() or 0) + 1:06d}"
    c = CotizacionVenta(numero=numero, cliente_id=body["cliente_id"], notas=body.get("notas"), usuario_id=current_user.id)
...
async def aceptar_cotizacion(cot_id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    c = (await db.execute(select(CotizacionVenta).where(CotizacionVenta.id == cot_id))).scalar_one_or_none()
...
async def rechazar_cotizacion(cot_id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    c = (await db.execute(select(CotizacionVenta).where(CotizacionVenta.id == cot_id))).scalar_one_or_none()
    if not c: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Cotización no encontrada")
    c.estado = "rechazada"; await db.flush(); return {"message": "Cotización rechazada"}

@router.post("/cotizaciones/{cot_id}/convertir")
async def convertir_cotizacion(cot_id: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    c = (await db.execute(select(CotizacionVenta).where(CotizacionVenta.id == cot_id).options(selectinload(CotizacionVenta.items)))).scalar_one_or_none()
    if not c: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Cotización no encontrada")
    if c.estado != "aceptada": raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Solo cotizaciones aceptadas")
    numero = await generar_numero_ov(db)
    p = PedidoVenta(numero=numero, cliente_id=c.cliente_id, cotizacion_id=c.id, usuario_id=current_user.id,
...
@router.get("/pedidos")
async def list_pedidos(estado: str | None = None, cliente_id: int | None = None, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    stmt = select(PedidoVenta).options(selectinload(PedidoVenta.cliente), selectinload(PedidoVenta.items).selectinload(ItemPedidoVenta.sku)).order_by(PedidoVenta.fecha_emision.desc())
    if estado:
...
async def get_pedido(pedido_id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    p = (await db.execute(select(PedidoVenta).where(PedidoVenta.id == pedido_id).options(selectinload(PedidoVenta.cliente), selectinload(PedidoVenta.items).selectinload(ItemPedidoVenta.sku)))).scalar_one_or_none()
...
async def create_pedido(body: dict, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    if not body.get("items"): raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Debe incluir ítems")
    c = (await db.execute(select(Cliente).where(Cliente.id == body["cliente_id"]))).scalar_one_or_none()
    if not c: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Cliente no encontrado")
    numero = await generar_numero_ov(db)
    p = PedidoVenta(numero=numero, cliente_id=body["cliente_id"], fecha_entrega=body.get("fecha_entrega"),
    ...
    p.subtotal = subtotal; p.impuesto_total = round(subtotal * 0.16, 2); p.total = round(subtotal + p.impuesto_total, 2)
    await db.flush(); return {"id": p.id, "numero": p.numero, "estado": p.estado.value}

@router.post("/pedidos/{pedido_id}/cancelar")
async def cancelar_pedido(pedido_id: int, body: dict = None, db: AsyncSession = Depends(get_db), _=Depends(require_admin)):
    p = (await db.execute(select(PedidoVenta).where(PedidoVenta.id == pedido_id))).scalar_one_or_none()
    if not p: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pedido no encontrado")
...
async def despachar_pedido(pedido_id: int, body: dict, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    try:
        await procesar_despacho(db=db, pedido_id=pedido_id, bodega_id=body["bodega_id"],
                                items_data=body["items"], usuario_id=current_user.id, nota=body.get("nota"))
...
async def list_despachos(db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    result = await db.execute(select(DespachoVenta).options(selectinload(DespachoVenta.pedido), selectinload(DespachoVenta.items).selectinload(ItemDespacho.item_pedido)).order_by(DespachoVenta.fecha.desc()))
...
async def devolver_pedido(pedido_id: int, body: dict, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    p = (await db.execute(select(PedidoVenta).where(PedidoVenta.id == pedido_id))).scalar_one_or_none()
    if not p: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pedido no encontrado")
    ...
    dev = DevolucionVenta(pedido_id=pedido_id, nota=body.get("nota"), usuario_id=current_user.id)
    ...
    for it in body.get("items", []):
        ...
        mov = MovimientoInventario(tipo=TipoMovimiento.ENTRADA, motivo=MotivoMovimiento.DEVOLUCION_CLIENTE, sku_id=ip.sku_id, bodega_id=body.get("bodega_id", 1), cantidad=it["cantidad_devuelta"], referencia=f"Devolución OV {p.numero}", usuario_id=current_user.id)
        db.add(mov); await actualizar_stock(db, ip.sku_id, body.get("bodega_id", 1), it["cantidad_devuelta"], TipoMovimiento.ENTRADA)
    await db.flush(); return {"message": "Devolución registrada"}

# ═══ FACTURACIÓN ════════════════════════════════════════

@router.post("/pedidos/{pedido_id}/facturar")
async def facturar_pedido(pedido_id: int, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    try: factura = await generar_factura(db, pedido_id, current_user.id)
    except ValueError as e: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
@router.get("/facturas")
async def list_facturas(db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    result = await db.execute(select(FacturaVenta).options(selectinload(FacturaVenta.cliente), selectinload(FacturaVenta.pedido)).order_by(FacturaVenta.fecha_emision.desc()))
...
async def get_factura(factura_id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    f = (await db.execute(select(FacturaVenta).where(FacturaVenta.id == factura_id)
...
async def pagar_factura(factura_id: int, db: AsyncSession = Depends(get_db), _=Depends(require_admin)):
    f = (await db.execute(select(FacturaVenta).where(FacturaVenta.id == factura_id))).scalar_one_or_none()
    if not f: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Factura no encontrada")
    f.estado = "pagada"; await db.flush(); return {"message": "Factura marcada como pagada"}


@router.post("/facturas/{factura_id}/anular")
async def anular_factura(factura_id: int, db: AsyncSession = Depends(get_db), _=Depends(require_admin)):
    f = (await db.execute(select(FacturaVenta).where(FacturaVenta.id == factura_id))).scalar_one_or_none()
    if not f: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Factura no encontrada")
