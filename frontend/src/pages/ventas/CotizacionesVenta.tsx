...
export default function CotizacionesVentaPage() {
  ...
  return (<div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}><h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--text)" }}>Cotizaciones de Venta</h2><button onClick={() => setShowForm(!showForm)} style={btnPri}>+ Nueva Cotización</button></div>
    ...
    : cotizaciones.map((c) => { const total = c.items.reduce((a, i) => a + i.precio_total, 0); return (<tr key={c.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
      ...
      <td style={td}>{c.estado === "pendiente" && <><button onClick={() => handleAction(c.id, "aceptar")} style={{ ...btnSm, color: "var(--success)", marginRight: 4 }}>Aceptar</button><button onClick={() => handleAction(c.id, "rechazar")} style={{ ...btnSm, color: "var(--danger)" }}>Rechazar</button></>}
