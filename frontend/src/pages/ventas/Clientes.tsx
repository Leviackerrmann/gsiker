...
export default function ClientesPage() {
  ...
  return (<div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}><h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--text)" }}>Clientes</h2><button onClick={() => { reset(); setShowForm(!showForm); }} style={btnPri}>+ Nuevo Cliente</button></div>
    ...
    <div style={card}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr style={{ borderBottom: "1px solid #e5e7eb" }}><th style={th}>Código</th><th style={th}>Nombre</th><th style={th}>Documento</th><th style={th}>Teléfono</th><th style={th}>Estado</th><th style={th}></th></tr></thead>
      <tbody>{loading ? <tr><td colSpan={6} style={{ ...td, textAlign: "center", color: "var(--text-muted)" }}>Cargando...</td></tr>
      : clientes.map((c) => (<tr key={c.id} style={{ borderBottom: "1px solid #f3f4f6" }}><td style={{ ...td, fontWeight: 600, color: "var(--primary)" }}>{c.codigo}</td><td style={td}>{c.nombre}</td><td style={td}>{c.documento || "-"}</td><td style={td}>{c.telefono || "-"}</td>
